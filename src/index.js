...
async function main() {
    ...
    findings = await analyzeAssets(build, {
      ...
      onCore: async ({ experiments }) => {
        ...
        if (ok) {
          ...
          try { await saveKnownIds(KNOWN_EXP, knownExp, 8000); } catch (e) { console.warn('early knownExp save', e.message); }
        } else {
  ...
  alreadyBuild = (typeof alreadyBuild !== 'undefined' && alreadyBuild) || (await wasBuildAnnounced(build.buildNumber));
