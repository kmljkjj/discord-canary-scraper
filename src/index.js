...
async function main() {
  const t0 = Date.now();
  console.log('=== Canary Pulse v11.9 ===');
  ...
  try {
    ...
    await fs.writeJson(path.join(DATA, 'apex_experiments.json'), apexList, {
      spaces: 2,
    });
    await fs.writeJson(
      path.join(DATA, 'experiments.json'),
      legacyList.length ? legacyList : allExps,
      { spaces: 2 },
    );
